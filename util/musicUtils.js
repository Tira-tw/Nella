...
module.exports = client => {
    ...
    client.joinVoiceChannel = async (channel) => {
        return new Promise(async (res, rej) => {
            ...
            if (oldConnection) return rej("我已經連接了: <#" + oldConnection.joinConfig.channelId + ">");
    ...
    client.leaveVoiceChannel = async (channel) => {
        return new Promise(async (res, rej) => {
            const oldConnection = getVoiceConnection(channel.guild.id);
            if (oldConnection) {
                if (oldConnection.joinConfig.channelId != channel.id) return rej("你知道我們在不同一個頻道嗎")
                try {
    ...
    client.sendQueueUpdate = async (guildId) => {
        ...
        const songEmbed = new Discord.MessageEmbed().setColor("FUCHSIA")
            ...
            .addField(`**時間:**`, `> \`${song.durationFormatted}\``, true)
